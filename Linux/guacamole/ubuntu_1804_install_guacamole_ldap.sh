...
read -p "Enter the Guacamole LDAP Host (Example: dc.domain.tld or 192.168.0.100):"  LDAP_HOST

if [ -z "$LDAP_HOST" ]
then
      
      echo "${RED}Guacamole LDAP Host cannot be empty ${RESET}"
      exit
fi

#read -p "Enter the Guacamole LDAP Host Encryption (Valid choices are: none, ssl, starttls):"  LDAP_ENCRYPTION
PS3='Select Guacamole LDAP Host Encryption: '
options=("none" "ssl" "starttls")
select LDAP_ENCRYPTION in "${options[@]}";

do
    case $LDAP_ENCRYPTION in
        "none")
        break
            ;;
...
then
      ...
      echo "${RED}Guacamole LDAP Host Port cannot be empty ${RESET}"
      exit
fi

read -p "Enter the Guacamole LDAP Bind User Distinguished Name (Example: CN=ldap_user,CN=Users,DC=domain,DC=tld):"  LDAP_BIND_DN

if [ -z "$LDAP_BIND_DN" ]
then
      
      echo "${RED}Guacamole LDAP Bind User Distinguished Name cannot be empty ${RESET}"
      exit
fi

read -p "Enter the Guacamole LDAP Bind User Password:"  LDAP_BIND_PASSWORD



read -p "Enter the Guacamole LDAP Users Distinguished Name (Example: DC=domain,DC=tld):"  LDAP_USER_DN

if [ -z "$LDAP_USER_DN" ]
then
      
      echo "${RED}Guacamole LDAP Users Distinguished Name cannot be empty ${RESET}"
      exit
fi
...
/bin/cp $SCRIPTPATH/templates/guacamole-docker-compose-template-ldap.yml /opt/guacamole/docker-compose.yml
...
/bin/cp $SCRIPTPATH/templates/.env-template-ldap /opt/guacamole/.env
...
echo "Configuring /opt/guacamole/.env file with Guacamole LDAP Host"
echo "[`date +%m/%d/%Y-%H:%M`] Configuring /opt/guacamole/.env file with Guacamole LDAP Host" >> $SCRIPTPATH/install_log-$TIMESTAMP.log

/bin/sed -i -e "s/LDAPHOST/${LDAP_HOST}/g" "/opt/guacamole/.env"

if [ $? -eq 0 ]; then
    echo "${GREEN}Done ${RESET}"
else
        echo "${RED}Error Configuring /opt/guacamole/.env file with Guacamole LDAP Host ${RESET}"
        echo "[`date +%m/%d/%Y-%H:%M`] Error Configuring /opt/guacamole/.env file with Guacamole LDAP Host" >> $SCRIPTPATH/install_log-$TIMESTAMP.log
        exit
fi

echo "Configuring /opt/guacamole/.env file with Guacamole LDAP Port"
echo "[`date +%m/%d/%Y-%H:%M`] Configuring /opt/guacamole/.env file with Guacamole LDAP Port" >> $SCRIPTPATH/install_log-$TIMESTAMP.log

/bin/sed -i -e "s/LDAPPORT/${LDAP_PORT}/g" "/opt/guacamole/.env"

if [ $? -eq 0 ]; then
    echo "${GREEN}Done ${RESET}"
else
        echo "${RED}Error Configuring /opt/guacamole/.env file with Guacamole LDAP Port ${RESET}"
        echo "[`date +%m/%d/%Y-%H:%M`] Error Configuring /opt/guacamole/.env file with Guacamole LDAP Port" >> $SCRIPTPATH/install_log-$TIMESTAMP.log
        exit
fi

echo "Configuring /opt/guacamole/.env file with Guacamole LDAP Encryption Type"
echo "[`date +%m/%d/%Y-%H:%M`] Configuring /opt/guacamole/.env file with Guacamole LDAP Encryption Type" >> $SCRIPTPATH/install_log-$TIMESTAMP.log

/bin/sed -i -e "s/LDAPENCRYPTION/${LDAP_ENCRYPTION}/g" "/opt/guacamole/.env"

if [ $? -eq 0 ]; then
    echo "${GREEN}Done ${RESET}"
else
        echo "${RED}Error Configuring /opt/guacamole/.env file with Guacamole LDAP Encryption Type ${RESET}"
        echo "[`date +%m/%d/%Y-%H:%M`] Error Configuring /opt/guacamole/.env file with Guacamole LDAP Encryption Type" >> $SCRIPTPATH/install_log-$TIMESTAMP.log
        exit
fi

echo "Configuring /opt/guacamole/.env file with Guacamole LDAP Bind User Distinguished Name"
echo "[`date +%m/%d/%Y-%H:%M`] Configuring /opt/guacamole/.env file with Guacamole LDAP Bind User Distinguished Name" >> $SCRIPTPATH/install_log-$TIMESTAMP.log

/bin/sed -i -e "s/LDAPBINDDN/${LDAP_BIND_DN}/g" "/opt/guacamole/.env"

if [ $? -eq 0 ]; then
    echo "${GREEN}Done ${RESET}"
else
        echo "${RED}Error Configuring /opt/guacamole/.env file with Guacamole LDAP Bind User Distinguished Name ${RESET}"
        echo "[`date +%m/%d/%Y-%H:%M`] Error Configuring /opt/guacamole/.env file with Guacamole LDAP Bind User Distinguished Name" >> $SCRIPTPATH/install_log-$TIMESTAMP.log
        exit
fi

echo "Configuring /opt/guacamole/.env file with Guacamole LDAP Bind User Password"
echo "[`date +%m/%d/%Y-%H:%M`] Configuring /opt/guacamole/.env file with Guacamole LDAP Bind User Password" >> $SCRIPTPATH/install_log-$TIMESTAMP.log

/bin/sed -i -e "s/LDAPBINDPASSWORD/${LDAP_BIND_PASSWORD}/g" "/opt/guacamole/.env"

if [ $? -eq 0 ]; then
    echo "${GREEN}Done ${RESET}"
else
        echo "${RED}Error Configuring /opt/guacamole/.env file with Guacamole LDAP Bind User Password ${RESET}"
        echo "[`date +%m/%d/%Y-%H:%M`] Error Configuring /opt/guacamole/.env file with Guacamole LDAP Bind User Password" >> $SCRIPTPATH/install_log-$TIMESTAMP.log
        exit
fi

echo "Configuring /opt/guacamole/.env file with Guacamole LDAP Users Distinguished Name"
echo "[`date +%m/%d/%Y-%H:%M`] Configuring /opt/guacamole/.env file with Guacamole LDAP Users Distinguished Name" >> $SCRIPTPATH/install_log-$TIMESTAMP.log

/bin/sed -i -e "s/LDAPUSERDN/${LDAP_USER_DN}/g" "/opt/guacamole/.env"
